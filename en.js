export default {
  buttons: {
    filter: "Filter",
    select_all: "Select All",
    deselect_all: "Deselect All",
    close: "",
    move_left: "",
    move_right: "",
    edit: "Edit",
    quit: "Quit",
    apply: "Yes", //确认
    cancel: "No", //取消
    heatmap: "Resource Heatmap",
  },
  edit: {
    mode_switch: "Enter/exit edit mode?", //是否进入/退出编辑模式
  },
  tabs: {
    by_category: "Category",
    by_item: "Item",
    by_search: "Search",
    resource_list: "Resources",
    setting: "Settings",
    user_data: "User Data",
    quick_setting: "Quick Settings",
    quick_setting_fuel_loot: "Fuel Loot",
    quick_setting_resource: "Resource",
    quick_setting_bus_route: "Monster Bus",
    quick_setting_show_players: "Show Players",
    quick_setting_pollution: "Pollution",
    quick_setting_popup_coordinate: "Coordinate Follow Pointer",
    quick_setting_disaster_area: "Chaos Weaver Disaster Area",
    area: "Areas",
    treasure: "Treasures",
    monster: "Explore",
    collection: "Collection",
    hint: "Hint",
    logged: "Data",
    guest: "Login",
    oneclick_delete_enable: "OneClick",
    oneclick_delete_disable: "Disable",
    oneclick_delete_badge: "In OneClick mode",
    oneclick_delete_hint1:
      "After turning it on, clicking on a marker will no longer display a pop-up window, but will directly delete or undelete the marker.",
    oneclick_delete_enable_hint: "OneClick deletion mode is enabled",
    oneclick_delete_disable_hint: "OneClick deletion mode is disabled",
    oneclick_delete_dialog_title: "Warning",
    oneclick_delete_dialog_content:
      "After turning on the one-click deletion mode, no pop-up window will appear when you click a marker on the map. Instead, the marker will be directly deleted or undelete. Do you want to continue turning it on?",
    oneclick_delete_dialog_close: "Close",
    oneclick_delete_dialog_confirm: "Confirm",
    batch_editing_enable: "Progress",
    batch_editing_disable: "Exit",
    batch_editing_hint:
      "You can batch delete or undelete all markers of one category. You need to log in to use it.",
    setting_btn_hint: "Map related settings",
    login_btn_hint: "Manage my data",
    back_to_categories: "Back to Categories",
    display_area: "Area Filter",
    select_item_category: "Select",
    related_items: "Related Items",
    locate_item: "Click on item to locate",
    search_resources: "Search Resources",
    input_resource_name: "Input name",
    locate: "Locate",
    // Miscellaneous UI strings
    any_name: "Any Words",
    keyword: "Keyword",
    locations: "Locations",
    locate_point: "Click to center the Location",
    input_item: "Please enter a Location Name",
    related_item: "Items",
    related_makers: "Markers",
    locate_marker: "Click to center the Marker",
    input_marker: "Please enter a Marker Name",
    related_resources: "Resources",
    locate_resource: "Click to center the Resource",
    input_resource_for_searching: "Please enter a Resource Name",
    follow_xingchuan: "Follow Xingchuan",
    bilibili: "BiliBili",
    tiktok: "TikTok",
    ad: "AD",
    ad2: "AD",
    jd: "Ad is not available in English version",
    daigan: "Ad is not available in English version",
    login_warring: "Please get the text from Discord Server bot first.",
    delete_all: "Delete All",
    resume_all: "Restore",
    exit_management_mode: "Exit",
    select_scenario: "Scenario",
    show_unreviewed_markers: "Unreviewed Markers",
    show_unreviewed_areas: "Unreviewed Areas",
    地点区域: "Locations",
    宝箱秘藏: "Secrets",
    事件怪物: "Monsters",
    收集要素: "Collections",
    异象轮盘: "Visional Wheel",
    地图提示: "Tips",
    无尽长梦: "Endless Dream",
    "七日世界：禁区": "OnceHuman: RaidZone",
    非正常收容: "Deviation: Survive",
    乐园岛: "Paradise Island",
    污染区: "Pollution Zones",
    tool_box: "Toolbox",
    about_website: "About"
  },
  floor_text: {
    roof: "( Roof )",
    at_floor: "( Floor {floor} )",
    underground: "( Floor {floor} Underground )",
    level: "( Lv. {level} )",
  },
  settings: {
    has_video: "Video Marker",
    deleted_marker: "Deleted Marker",
    map_area_style: "Map Area Style",
    monster_bus_route: "Monster Bus Route",
    marker_text: "Marker Text",
    resource_area: "Resource Area",
    last_updated_datetime: "Marker Updated Time",
    icon_drawing_method:
      "Icon Drawing Method (Canvas is Recommended when the icon is broken)",
    dark_mode: "Dark Mode",
    language: "Language",
    auto_query: "Query when the Categories Change",
    auto_draw_marker_after_search: "Display the Search Result on the Map",
    show: "Show",
    hide: "Hide",
    line_style: "Line Style",
    area_style: "Fill Style",
    wasm: "Wasm (Best Performance)",
    canvas: "Canvas (Best Compatibility)",
    on: "On",
    off: "Off",
    draw_text_mode: "Marker Text Style",
    draw_text_mode_1: "Complete",
    draw_text_mode_2: "Simplified",
    deleted_marker_style: "Deleted Marker Style",
    deleted_marker_style_1: "Cross",
    deleted_marker_style_2: "Translucent",
    deleted_marker_style_3: "Hide",
  },
  user_data: {
    local_storage: "Local Data",
    count: "no records | {count} record | {count} records",
    local_storage_tutorial: "Tutorial",
    backup: "Backup & Restore",
    local_storage_text:
      "You can delete any marker without login. But the data will only storage at current browser. You can backup the data and transfer to another web browser.",
    backup_placeholder: "Datas",
    backup_button: "Backup",
    restore_button: "Restore",
    cloud_storage: "Cloud Data",
    clear_all_data: "Delete All Data",
    restore_hint:
      "Tip: This operation will merge local data with imported data. If you are logged in, the data will also be synchronized to the cloud. Do you want to continue?",
    delete_hint:
      "Are you sure you want to delete all data? If you are already logged in, the cloud data will also be cleared. Do you want to continue?",
    wechat_login: "Wechat",
    wechat_mp_login: "Wechat2",
    discord_login: "Discord",
    login_tutorial: "Tutorial",
    login_tutorial_text:
      "After logging in, the local and cloud data will be automatically synchronized, and you can continue to use it on any browser and device. You can also use it normally without logging in, but the data can only be stored in the local browser.",
    not_login: "Not logged in",
    login_success: "You have successfully logged in",
    data_is_syncing: "Data is Syncing",
    last_syncing_time: "Last Syncing Time: ",
    logout: "Log out",
  },
  marker_dialog: {
    no_marker_img: "No Images",
    delete_markder: "Delete Marker",
    undelete: "Undo Delete",
    delete_only: "Delete Only",
    hide_category: "Hide {category}",
    no_update_time: "No update time",
    rewards: "Rewards",
    close: "Close",
    detail: "Detail",
    operation: "Operation",
    timer_title: "Crate Reset Timer",
    timer_title_not_login: "(Need Login) Crate Reset Timer",
    deviant_timer_title: "Deviant Reset Timer",
    to_login: "Login to Unlock Timer Feature",
    comment: "Comments",
    auto_set_timer: "Auto Set Timer",
    auto_set_timer_not_login: "(Need Login) Auto Set Timer",
    timer_tutorial: "Timer Tutorial",
    sharing: "Sharing",
    session_script: "Currently available at: ",
    hint: "[ Hint ]",
    sharing_hint:
      "The shared link will automatically display the marker when opened.",
    download_qr_code: "Download",
    sharing_link: "Link",
    copy: "Copy",
    update: "Updated",
    guide: "Guide",
    donate: "Donate",
    auto_reset_help_title: "Help",
    auto_reset_help_text:
      "The fixed refresh time for weapon crates and gear crates starts at 00:00:00 and refreshes every 4 hours on the hour. If you still can't open the crates when you reach the refresh time, just log in again.",
    auto_reset_help_ok: "OK",
    click_to_copy: "Click to Copy",
    copied_successfully: "Copied Successfully",
    common_hint: "General Tips",
    batch_import: "Batch Import",
    navigation_settings: "Navigation Settings",
    contributors: "Contributors",
  },
  donate: {
    payment: "Please scan the QR code using WeChat",
    text: "Your support can help the website continue to operate and update more features",
    warning: "Minors are prohibited from donating",
  },
  scenario: {
    not_available: "Not Available",
    current: "Current",
    available: "Available",
    coming: "Coming",
    scenarios: "Maps",
  },
  languages: {
    dialog_title: "Select Language",
    dialog_close: "Close",
    contributors: "Contributors",
    help_1: "Didn't find the language you were looking for?",
    help_2: "Click here to help improve translations.",
    default: "Default",
  },
  path_planning: {
    path_planning: "Path Planning",
    start: "Start Point",
    end: "End Point",
    select_start: "Select Start Point",
    select_end: "Select End Point",
    tips: "Tip: Right click on the map to quickly set the points",
    clear: "Clear Path",
    submit: "Query Path",
    cancel: "Clear",
    placeholder: "Select on the map",
  },
  heatmap: {
    no_data: "No Data",
    hint: "The darker the heat map color, the more resources",
    clear: "Clear Heatmap",
  },
  exploration_note: {
    "探索笔记": "Exploration Notes",
    "分享中心": "Sharing Hub",
    "搜索笔记标题或说明": "Search note titles or descriptions",
    "搜索": "Search",
    "找到 {count} 条相关笔记": "Found {count} matching notes",
    "发现玩家分享的跑图与采集路线": "Discover exploration and gathering routes shared by players",
    "刷新": "Refresh",
    "清除笔记": "Clear Note",
    "笔记已清除": "Note cleared",
    "未设置昵称": "Nickname not set",
    "作者暂未填写笔记说明": "The author has not added a description yet",
    "使用 {count} 次": "Used {count} times",
    "更新于 {time}": "Updated {time}",
    "使用笔记": "Use Note",
    "复制分享链接": "Copy Share Link",
    "编辑": "Edit",
    "暂无公开笔记": "No public notes yet",
    "创建我的笔记": "Create My Note",
    "登录后才可以编辑和保存探索笔记。": "Log in to edit and save exploration notes.",
    "立即登录": "Log In Now",
    "基础": "Basics",
    "笔记标题": "Note Title",
    "例如：黑原油田快速采集路线": "Example: Blackfell Oil Fields Quick Gathering Route",
    "路线说明": "Route Description",
    "选填": "Optional",
    "写下路线用途、注意事项或采集顺序": "Describe the route's purpose, precautions, or gathering order",
    "所属剧本": "Scenario",
    "请选择剧本": "Select a scenario",
    "打开后的地图中心": "Map Center When Opened",
    "取当前中心": "Use Current Center",
    "下一步：选择地图标记": "Next: Select Map Markers",
    "标记": "Markers",
    "附带地图分类": "Include Map Categories",
    "使用笔记时覆盖当前勾选": "Replace current selections when using the note",
    "已选 {count} 项": "{count} selected",
    "全选": "Select All",
    "清空": "Clear",
    "此笔记不会改变玩家的地图分类筛选": "This note will not change the player's map category filters",
    "绘制": "Draw",
    "添加元素": "Add Elements",
    "图标": "Icon",
    "标记图标": "Marker Icon",
    "标记文本（选填）": "Marker Label (Optional)",
    "显示在图标下方": "Displayed below the icon",
    "从现有图标中选择": "Select from existing icons",
    "文字": "Text",
    "文字内容": "Text Content",
    "输入要显示的文字": "Enter the text to display",
    "字号": "Font Size",
    "颜色": "Color",
    "加粗": "Bold",
    "背景": "Background",
    "显示描边背景": "Show outlined background",
    "不随地图缩放改变大小": "Keep size while zooming",
    "路线": "Route",
    "手绘": "Freehand",
    "线条颜色": "Line Color",
    "粗细": "Width",
    "完成绘制": "Finish Drawing",
    "开始在地图添加": "Start Adding to Map",
    "撤销一步": "Undo Last Step",
    "元素管理 ({count})": "Manage Elements ({count})",
    "可单独隐藏、显示或删除笔记元素": "Hide, show, or delete note elements individually",
    "确认清空": "Confirm Clear",
    "取消": "Cancel",
    "清空全部元素": "Clear All Elements",
    "此操作会删除全部图标、文字、路线和手绘，且无法撤销。确定继续吗？": "This will permanently delete all icons, text, routes, and freehand drawings. Continue?",
    "已隐藏": "Hidden",
    "显示中": "Visible",
    "保存": "Save",
    "改名": "Rename",
    "显示": "Show",
    "隐藏": "Hide",
    "删除": "Delete",
    "确定删除“{name}”吗？": "Delete “{name}”?",
    "还没有添加任何元素": "No elements have been added yet",
    "分享": "Share",
    "笔记概览": "Note Overview",
    "分类": "Categories",
    "线条": "Lines",
    "发布到分享中心": "Publish to Sharing Hub",
    "其他玩家可在探索列表中查看和使用": "Other players can view and use it in the exploration list",
    "保存前请先在“基础”中设置地图中心点。": "Set the map center under “Basics” before saving.",
    "保存并生成笔记码": "Save and Generate Note Code",
    "分享链接": "Share Link",
    "复制": "Copy",
    "我的分享": "My Shares",
    "登录后可以管理自己创建的笔记。": "Log in to manage the notes you created.",
    "共 {count} 条笔记": "{count} notes total",
    "已发布": "Published",
    "未发布": "Unpublished",
    "暂未填写说明": "No description yet",
    "继续编辑": "Continue Editing",
    "复制链接": "Copy Link",
    "确认删除": "Confirm Delete",
    "确定删除“{name}”吗？删除后无法恢复。": "Delete “{name}”? This cannot be undone.",
    "还没有创建笔记": "No notes created yet",
    "编辑教程": "Editing Guide",
    "探索笔记可保存地图分类、图标点位、文字、路线和手绘，并通过笔记码分享给其他玩家。": "Exploration notes can save map categories, icon markers, text, routes, and freehand drawings, and can be shared with other players using a note code.",
    "填写基础信息": "Enter Basic Information",
    "进入“创建我的笔记”，填写标题、说明，选择剧本，并取当前地图中心。": "Open “Create My Note,” enter a title and description, select a scenario, and use the current map center.",
    "选择地图标记": "Select Map Markers",
    "在“标记”中选择使用笔记时用于覆盖当前筛选的内置分类。": "Under “Markers,” select the built-in categories that replace the current filters when the note is used.",
    "添加绘制元素": "Add Drawing Elements",
    "在“绘制”中选择图标、文字、路线或手绘，点击开始后直接操作地图。": "Under “Draw,” choose an icon, text, route, or freehand tool, then start and interact directly with the map.",
    "管理元素": "Manage Elements",
    "元素管理支持改名、隐藏、重新显示、单独删除以及清空全部元素。": "Element management supports renaming, hiding, showing again, deleting individual elements, and clearing all elements.",
    "保存与发布": "Save and Publish",
    "在“分享”中保存生成笔记码；开启发布后会出现在分享中心。": "Save under “Share” to generate a note code; published notes appear in the Sharing Hub.",
    "继续维护": "Continue Managing",
    "在“我的分享”中找到笔记，可再次编辑、复制链接或删除。": "Find notes under “My Shares” to edit them again, copy their links, or delete them.",
    "绘制操作": "Drawing Controls",
    "图标和文字：开始添加后，点击地图放置。": "Icons and text: After starting, click the map to place them.",
    "路线：逐点点击地图，双击结束一条路线。": "Routes: Click points on the map, then double-click to finish a route.",
    "手绘：在地图上按住鼠标拖动，松开完成。": "Freehand: Hold and drag on the map, then release to finish.",
    "图标点位": "Icon Marker",
    "文字标记": "Text Marker",
    "采集路线": "Gathering Route",
    "自由手绘": "Freehand Drawing",
    "尚未设置": "Not Set",
    "选择图标并按需填写标记文本，点击开始后在地图放置点位。": "Select an icon, optionally enter a label, then start and place the marker on the map.",
    "设置文字样式后点击开始，再点击地图放置文字。": "Set the text style, start drawing, then click the map to place the text.",
    "在地图上逐点点击绘制路线，双击结束当前路线。": "Click points on the map to draw a route, then double-click to finish it.",
    "在地图上按住鼠标并拖动，松开后完成手绘。": "Hold and drag on the map, then release to finish the freehand drawing.",
    "请先登录": "Please log in first",
    "请先选择图标": "Select an icon first",
    "请先输入文字": "Enter text first",
    "元素名称不能为空": "Element name cannot be empty",
    "探索笔记已保存": "Exploration note saved",
    "未知时间": "Unknown time",
    "无权编辑该笔记": "You do not have permission to edit this note",
    "笔记已删除": "Note deleted",
    "分享链接已复制": "Share link copied",
    "链接已复制": "Link copied"
  },
  categories: {
    // Areas
    传送塔: "Teleporter",
    巨楔: "Monolith",
    主城: "Settlement",
    研究所: "Lab",
    大型资源点: "Town",
    小型资源点: "Village",
    收容井: "Securement Silo",
    位面锚: "Rift Anchor",
    空间锚点: "Rift Anchor",
    观景点: "View Point",
    // Treasures
    神秘宝藏: "Mystical Crate", //Updated Name Ingame One
    神秘宝箱: "Mystical Crate", //Updated Name Ingame One
    武器箱: "Weapon Crate",
    装备箱: "Gear Crate",
    隐藏宝箱: "Hidden Crate",
    秘藏宝箱: "Hidden Hoard Crate",
    宝箱怪: "Morphic Crate",
    储物箱: "Storage Crate",
    // Explore
    精英怪: "Elite",
    迷雾区域: "Fog Event",
    防守工坊: "Workshop",
    移动房屋: "Moving House",
    限时事件: "Red Event",
    灭绝者: "Chaos Weaver",
    "灭绝者(二阶段)": "Chaos Weaver(Stage 2)",
    "灭绝者(三阶段)": "Chaos Weaver(Stage 3)",
    "冰灭绝者(一阶段)": "Ice Chaos Weaver(Stage 1)",
    "冰灭绝者(二阶段)": "Ice Chaos Weaver(Stage 2)",
    "冰灭绝者(三阶段)": "Ice Chaos Weaver(Stage 3)",
    "冰灭绝者(二/三阶段)": "Ice Chaos Weaver(Stage 2/3)",
    "冰灭绝者(一/四阶段)": "Ice Chaos Weaver(Stage 1/4)",
    "冰灭绝者(二/四阶段)": "Ice Chaos Weaver(Stage 2/4)",
    "冰灭绝者(三/四阶段)": "Ice Chaos Weaver(Stage 3/4)",
    "冰灭绝者(二/三/四阶段)": "Ice Chaos Weaver(Stage 2/3/4)",
    "冰灭绝者(四阶段)": "Ice Chaos Weaver(Stage 4)",
    "火灭绝者(二阶段)": "Fire Chaos Weaver(Stage 2)",
    "火灭绝者(三阶段)": "Fire Chaos Weaver(Stage 3)",
    "火灭绝者(四阶段)": "Fire Chaos Weaver(Stage 4)",
    "火灭绝者(一/四阶段)": "Fire Chaos Weaver(Stage 1/4)",
    "火灭绝者(二/四阶段)": "Fire Chaos Weaver(Stage 2/4)",
    "火灭绝者(三/四阶段)": "Fire Chaos Weaver(Stage 3/4)",
    "火灭绝者(二/三/四阶段)": "Fire Chaos Weaver(Stage 2/3/4)",
    // Collection
    武器配件: "Attachment", // "Weapon Attachments" is too long and breaks the UI
    食谱: "Recipe",
    收容物: "Deviant",
    诡谲画作: "Mysterious Painting",
    矿物点: "Mineral",
    采集点: "Material",
    种子箱: "Seed Chest",
    调味料箱: "Spice Chest",
    种子香料: "Seed & Spice",
    饮料机: "Drink Machine",
    随机素材箱: "Random Chest",
    抗菌剂箱: "Medicine Chest",
    // PvP
    回响石: "Echo Stone",
    // Hint
    留言: "Hint",
    隐藏任务: "Side Quest",
    支线任务: "Side Quest",
    商人: "Merchant",
    宜居地点: "Livable Location",
    // Regions
    代顿湿地: "Dayton Wetlands",
    破碎三角洲: "Broken Delta",
    垩峰山: "Chalk Peak",
    铁锈河: "Iron River",
    孤狼荒原: "Red Sands",
    黑心地带: "Blackheart",
    玛瑙冻原: "Onyx Tundra",
    维纳峡湾: "Vena Fjord",
    余烬滩: "Ember Strand",
    // Routes
    // No need to translate the fuel count number.
    怪物巴士路线: "Monster Bus",
    便携混合燃油: "Portable Mixed Fuel",
    // Areas
    污染区: "Contaminated Area",
    污染值: "Pollution Value",
    建筑无法幸存: "Building cannot survive",
    建筑会受损: "Building will be damaged",
    被发现范围: "Alert Range",
    // Teleporters - 传送塔
    猎犬岛传送塔: "Teleportation Tower",
    望岛镇传送塔: "Teleportation Tower",
    离岸广场传送塔: "Teleportation Tower",
    临溪东部传送塔: "Teleportation Tower",
    贪欲巨楔传送塔: "Monolith of Greed Teleportation Tower",
    代顿检查点传送塔: "Teleportation Tower",
    高岸南部传送塔: "Teleportation Tower",
    西格玛传送塔: "Teleportation Tower",
    暖阳中部传送塔: "Teleportation Tower",
    三角洲南部传送塔: "Teleportation Tower",
    盖亚山区传送塔: "Teleportation Tower",
    盖亚巨楔传送塔: "Gaia Cliff Monolith Teleportation Tower",
    代顿北部传送塔: "Teleportation Tower",
    高地西部传送塔: "Teleportation Tower",
    盖亚西部传送塔: "Teleportation Tower",
    朽鞍镇传送塔: "Teleportation Tower",
    银滩北部传送塔: "Teleportation Tower",
    翠湖南部传送塔: "Teleportation Tower",
    翠湖北部传送塔: "Teleportation Tower",
    阿尔法传送塔: "Teleportation Tower",
    月湖传送塔: "Teleportation Tower",
    庄园东部传送塔: "Teleportation Tower",
    尖牙巨楔传送塔: "Monolith of Thirst Teleportation Tower",
    蜿蜒山区传送塔: "Teleportation Tower",
    波光北部传送塔: "Teleportation Tower",
    黯梦巨楔传送塔: "Mirage Monolith Teleportation Tower",
    风角边境传送塔: "Teleportation Tower",
    谷地南部传送塔: "Teleportation Tower",
    西塔传送塔: "Teleportation Tower",
    黑原中部传送塔: "Teleportation Tower",
    遗忘巨楔传送塔: "Forsaken Monolith Teleportation Tower",
    悬巢北部传送塔: "Teleportation Tower",
    悬巢南部传送塔: "Teleportation Tower",
    内部传送塔: "Teleportation Tower",
    巨人阶梯传送塔: "Teleportation Tower",
    黑心内弯传送塔: "Blackheart's Bay Teleportation Tower",
    //Way of Winter - 雪国人生
    //"望海传送塔": "Teleportation Tower", //Way of Winter - 10/19/24
    冻火湾传送塔: "Frost Forge Teleportation Tower", //Way of Winter - 10/19/24
    冻火北部传送塔: "Frost North Teleportation Tower", //Way of Winter - 10/19/24
    鲸骨湾传送塔: "Whalebone Bay Teleportation Tower", //Way of Winter - 10/19/24
    山阴传送塔: "Mountain Shade Teleportation Tower", //Way of Winter - 10/19/24
    水畔传送塔: "Waterside Teleportation Tower", //Way of Winter - 10/19/24
    港区传送塔: "Harbor District Teleportation Tower", //Way of Winter - 10/19/24
    绿野传送塔: "Green Field Teleportation Tower", //Way of Winter - 10/19/24
    乐园传送塔: "Wish Land Teleportation Tower", //Way of Winter - 10/19/24
    望海传送塔: "Oceanview Teleportation Tower", //Way of Winter - 10/19/24
    林场传送塔: "Woodland Ranch Teleportation Tower", //Way of Winter - 10/19/24
    慕斯市传送塔: "Moose City Teleportation Tower", //Way of Winter - 10/19/24
    潮间地传送塔: "Tidal Bore Teleportation Tower", //Way of Winter - 10/19/24
    黑石东部传送塔: "East Blackstone Teleportation Tower", //Way of Winter - 10/19/24
    黑石北部传送塔: "North Blackstone Teleportation Tower", //Way of Winter - 10/19/24
    冻原中部传送塔: "Central Tundra Teleportation Tower", //Way of Winter - 10/19/24
    大河警戒区传送塔: "Taiga Exclusion Zone Teleportation Tower", //Way of Winter - 10/19/24
    母巢巨穴传送塔: "Hiverail Cave Teleportation Tower", //Way of Winter - 10/19/24
    破碎岩礁传送塔: "Broken Reefs teleportation Tower", //Way of Winter - 10/19/24
    雪崩巨楔传送塔: "Avalanche Monolith Teleportation Tower", //Way of Winter - 10/21/24
    雪崖传送塔: "Snow Cliffs Teleportation Tower", //Way of Winter - 10/21/24
    冻火西部传送塔: "Frost West Teleportation Tower", //Way of Winter - 10/21/24
    烟袋群岛传送塔: "Pipe Island Teleportation Tower", //Way of Winter - 10/21/24
    蛇眼山口传送塔: "SnakeEye Pass Teleportation Tower",
    蛇腹传送塔: "SnakeBelly Teleportation Tower",
    蛇尾传送塔: "SnakeTail Teleportation Tower",
    熟土区传送塔: "Ripe Land Teleportation Tower",
    火喉北部传送塔: "Fire Throat North Teleportation Tower",
    火喉东部传送塔: "Fire Throat East Teleportation Tower",
    火喉飞升传送塔: "Fire Throat Rise Teleportation Tower",
    火喉根基传送塔: "Fire Throat Base Teleportation Tower",
    灰落传送塔: "AshenTon Teleportation Tower",
    寂火传送塔: "SilentFire Teleportation Tower",
    灼风传送塔: "BlazeWind Teleportation Tower",
    硫磺池传送塔: "Sulfur Pool Teleportation Tower",
    炽尘巨楔传送塔: "FireDust Teleportation Tower",

    // Monoliths - 巨楔
    贪欲巨楔: "Monolith of Greed",
    盖亚巨楔: "Gaia Cliff Monolith",
    黯梦巨楔: "Mirage Monolith",
    尖牙巨楔: "Monolith of Thirst",
    遗忘巨楔: "Forsaken Monolith",
    //Way of Winter - 雪国人生
    冻原巨楔: "Tundra Monolith", //Way of Winter - 10/19/24
    雪崩巨楔: "Avalanche Monolith", //Way of Winter - 10/19/24
    炽尘巨楔: "Firedust Monolith",
    // Settlements - 主城
    旅人营地: "Deadsville",
    迈尔集市: "Meyer's Market",
    秽水营地: "Greywater Camp",
    深草旅店: "Tall Grass Inn",
    黑原镇: "Blackfell",
    //Way of Winter - 雪国人生
    遗忘之地: "Oblivion", //Way of Winter - 10/19/24
    熊栖雪场: "Bear's Den", //Way of Winter - 10/19/24
    鲸骨乡: "Whale Bone", //Way of Winter - 10/19/24
    冰屋营地: "Ice House Camp", //Way of Winter - 10/19/24
    不落号坠落点: "Unfallen Crash Site",

    // Labs - 研究所
    生命进化研究所: "HP Evolution Institute",
    "”黑域“之门": "Entrance of the Zone of Haze", //Way of Winter - 10/21/24

    // Towns - 大型资源点
    中途镇: "Throughville",
    腐烂庄园: "Rotten Manor",
    橘子镇: "Citrus County",
    望岛镇: "Overlook Town",
    苏德兰果园: "Sutherland Family Orchard",
    离岸广场: "Coastside Plaza",
    临溪: "Brookham",
    代顿医院: "Dayton Hospital",
    贪欲巨楔警戒区: "Monolith of Greed Danger Zone",
    赫斯特工业: "Hearst Industries",
    海港镇: "Harborside",
    猎犬岛哨站: "Wild Dog Isle Outpost",
    暖阳中学: "Sunbury Middle School",
    暖阳镇: "Sunbury",
    苏德兰化工厂: "Sutherland Chemical Plant",
    高岸镇: "High Banks",
    废弃货场: "Junkyard",
    盖亚研究所遗址: "Gaia Research Center Ruins",
    东部铁路枢纽: "Eastern Railway Junction",
    盖亚巨楔警戒区: "Gaia Cliff Monolith Danger Zone",
    高地: "Highland",
    冶炼厂污染点: "Refinery Pollution Plant",
    秽水工业区: "Greywater Industrial Zone",
    阿卡尔克镇: "Alkirk",
    东黑原枢纽: "East Blackfell Junction",
    波光镇: "Rippleby",
    里奇收容点: "Ricci Securement Point",
    黯梦巨楔警戒区: "Mirage Monolith Danger Zone",
    盖亚军事基地: "Gaia Military Base",
    朽鞍镇: "Rotten Saddle",
    银滩度假区: "Silvershore Resort",
    碧湖山镇: "Greenlake Hill",
    霍特镇: "Holt Town",
    熔炉巢穴: "Furnace Lair",
    常青藤酒庄: "Evergreen Vineyard",
    阿尔法研究所: "Alpha Institute",
    尖牙巨楔警戒区: "Monolith of Thirst Danger Zone",
    交错现实研究所: "Alternative Reality Research Institute",
    黑原油田: "Blackfell Oil Fields",
    黑原陷落区: "Blackfell Fallen Zone",
    绿荫镇: "Evergreen",
    悬巢: "Fort Eyrie",
    "73号源质分离点": "73 Source Extraction Point",
    太阳农场: "Sunshine Farm",
    白崖基地: "White Cliff",
    //Way of Winter - 雪国人生
    灼风气象站: "Scorchwind Observatory", //Way of Winter - 10/19/24
    炎神研究所: "Fire God Research Center", //Way of Winter - 10/19/24
    圣城前哨: "Firethroat Outskirts", //Way of Winter - 10/19/24
    火喉下城: "Firethroat Fortress", //Way of Winter - 10/19/24
    灰落车站: "Ashenton Station", //Way of Winter - 10/19/24
    熟土农庄: "Ripe Land Farm", //Way of Winter - 10/19/24
    蛇尾滑索营地: "Serpent's Tail Camp", //Way of Winter - 10/19/24
    火喉弃民街: "Firethroat Slums", //Way of Winter - 10/19/24
    燃尽之地: "Charred Wasteland", //Way of Winter - 10/19/24
    废弃提炼厂: "Abandoned Refinery", //Way of Winter - 10/19/24
    攀火祭祀场: "Fire Aspect", //Way of Winter - 10/19/24
    望火港: "Firewatch Harbor", //Way of Winter - 10/19/24
    烟袋度假区: "Tabcco Resort", //Way of Winter - 10/19/24
    黑石灯塔: "Obsidian Lighthouse", //Way of Winter - 10/19/24
    异火码头: "Pyro Harbor", //Way of Winter - 10/21/24
    无明港: "Lightless Port", //Way of Winter - 10/21/24
    蛇眼瞭望点: "Snakeeye Lookout", //Way of Winter - 10/21/24
    蛇腹洞穴: "Snakebelly Cave", //Way of Winter - 10/21/24
    纵队巡逻点: "Guerilla Partrol Point", //Way of Winter - 10/21/24
    硫磺池采伐区: "Sulphur Quarry", //Way of Winter - 10/21/24
    温泉民宿: "Hot Spring Inn", //Way of Winter - 10/21/24
    火喉圣域: "Firethroat Fortress", //Way of Winter - 10/21/24 (Might not be accurate) <--- Come Back
    冻火湾: "Frost Forge", //Way of Winter - 10/21/24
    避火者地堡: "Flame Sanctuary", //Way of Winter - 10/21/24
    炽尘巨楔警戒区: "Firedust Monolith Danger Zone", //Way of Winter - 10/21/24
    "“母巢”收容保护区": "Hiverail Restricted Area", //Way of Winter - 10/21/24
    雪崩巨楔警戒区: "Avalanche Monolith Danger Zone", //Way of Winter - 10/21/24
    雪原气象站: "Snowfield Checkpoint", //Way of Winter - 10/21/24
    悬空哨卡: "Suspended Checkpoint", //Way of Winter - 10/21/24
    冻原巨楔警戒区: "Tundra Monolith Danger Zone", //Way of Winter - 10/21/24
    检测点653: "Inspection Point 653", //Way of Winter - 10/21/24
    "“大河”收容场": "“Great River“ Shelter Zone", //Way of Winter - 10/21/24
    罗塞塔巡逻站: "Rosetta Patrol Station", //Way of Winter - 10/21/24
    慕斯市食品厂: "Moose City Food Factory", //Way of Winter - 10/21/24
    慕斯市: "Moose City", //Way of Winter - 10/21/24 (Partial Guess Based on Teleporter)
    慕斯市郊废屋: "Abandoned House on the Outskirts of Moose City", //Way of Winter - 10/21/24
    栖木岭观景台: "Perching Tree Prospect Observation Desk", //Way of Winter - 10/21/24
    栖木缆车站: "Perching Tree Cable Car Station", //Way of Winter - 10/21/24
    藏火岩洞: "Firehide Cavern", //Way of Winter - 10/21/24
    黑石要塞: "Blackstone Fortress", //Way of Winter - 10/21/24
    "“心愿奇境”西园区": "Wishland West", //Way of Winter - 10/21/24
    "“心愿奇境”东园区": "Wishland East", //Way of Winter - 10/21/24
    乐园东入口: "Wishland East Entrance", //Way of Winter - 10/21/24
    乐园西入口: "Wishland West Entrance", //Way of Winter - 10/21/24
    滑索洞穴营地: "Zipline Cave Camp", //Way of Winter - 10/21/24
    融冰港: "Icemelt Harbor", //Way of Winter - 10/21/24
    菲斯库尔港: "Fiskur Harbor", //Way of Winter - 10/21/24
    影日洞穴: "Sunshroud Cave", //Way of Winter - 10/21/24
    灰落市: "Ashenton", //Way of Winter - 10/21/24

    // Villages
    中途镇变电站: "Throughville Substation",
    检测点3771: "Inspection Point 3771",
    翻车鱼岛游客中心: "Mola Island Visitor Center",
    海岬庄园: "Cape Manor",
    小码头: "Petit Harbor",
    艾尔汽修店: "Aiden's Auto Repair",
    艾尔的隐蔽所: "Aiden's Hideout",
    检测点7802: "Inspection Point 7802",
    临溪警戒所: "Brookham Guardhouse",
    代顿货栈: "Dayton Warehouse",
    崖下营地: "Cliffside Camp",
    北岸营地: "North Shore Camp",
    赫斯特: "Hearst",
    赫斯特滩涂: "Hearst Mudflats",
    湖心野营地: "Lakeheart Camp",
    迈尔: "Meyer",
    高岸巢穴: "Highbank Nest",
    废弃隐蔽所: "Abandoned Hideout",
    骑士修车店: "Knight Vehicle Repairs",
    山脊中转站: "Ridge Transfer Station",
    山脊营地: "Ridge Camp",
    暖阳农庄: "Sunbury Farm",
    瀑布广场: "Waterfall Plaza",
    沉船坟场: "Ship Graveyard",
    沉没宅院: "Sunken House",
    检测点024: "Inspection Point 024",
    检测点9043: "Inspection Point 9043",
    "TAB-6A营地": "Tab-6A Camp",
    高地居住区: "Highland Residential Zone",
    洗劫一空的营地: "Ransacked Camp",
    远眺伐木场: "Overlook Lumberyard",
    污染检测点77: "Pollution Inspection Point 77",
    河心码头: "Riverhart Harbor",
    河畔纪念品店: "Riverside Souvenirs",
    枯竭油井: "Depleted Oil Well",
    蜿蜒山种植园: "Winding Ridge Farmland",
    浅滩公园: "Shoal Park",
    波光镇集市: "Rippleby Market",
    风车阵列: "Wind Farm",
    秘港: "Secret Harbor",
    检测点023: "Inspection Point 023",
    克里夫顿: "Clifton",
    海风疗养地: "Breezy Shores Resort",
    诺顿农场: "Norton Farm",
    高岸检测点: "Highbank Inspection Point",
    私酿庄园: "Brewery Estate",
    老大的乐园: "The Boss's Playground",
    霍特电站: "Holt Power Station",
    帕斯利广场: "Paisley Plaza",
    山麓别墅: "Hilly Homestead",
    盖亚通讯阵列: "Gaia Communication Array",
    检测点266: "Inspection Point 266",
    狂热信徒聚落: "Believer Colony",
    戴维斯角: "Davis Point",
    鞍前营地: "Saddlefront Camp",
    渔人码头: "Fisherman Wharf",
    格尔峰电站: "Mount Guel Power",
    检测点091: "Inspection Point 091",
    妮娜奶奶的水果餐吧: "Grandma Nina's Diner",
    检测点384: "Inspection Point 384",
    矿镇: "Mining Town",
    翠湖游客中心: "Greenlake Visitor Center",
    //Way of Winter - 雪国人生
    搁浅滩涂: "Stranding Mudflats", //Way of Winter - 10/19/24
    海边旧屋: "Seaside Hut", //Way of Winter - 10/21/24
    洞口营地: "Cave Mouth Camp", //Way of Winter - 10/21/24
    破败码头: "Derelict Warf", //Way of Winter - 10/21/24
    幽谷民居: "Vale Residence", //Way of Winter - 10/21/24
    废弃水塔: "Abandoned Water Tower", //Way of Winter - 10/21/24
    瞭望处: "Lookout", //Way of Winter - 10/21/24
    松林猎场: "Pine Hunting Ground", //Way of Winter - 10/21/24
    纵队前哨: "Guerilla Outpost", //Way of Winter - 10/21/24
    废弃避难所: "Abandoned Shelter", //Way of Winter - 10/21/24
    干涸码头: "Dried Harbor", //Way of Winter - 10/21/24
    林场: "Forest", //Way of Winter - 10/21/24
    潮间地农场: "Tidal Zone Farm", //Way of Winter - 10/21/24
    食品厂货栈: "Sweet Factory Warehouse", //Way of Winter - 10/21/24
    悬索营地: "Cable Camp", //Way of Winter - 10/21/24
    栈道木屋: "Boardwalk Cabin", //Way of Winter - 10/21/24
    废弃探险营地: "Abandoned Expedition Camp", //Way of Winter - 10/21/24
    悬索货栈: "Cable Warehouse", //Way of Winter - 10/21/24
    荒废农庄: "Abandoned Farm", //Way of Winter - 10/21/24
    鲸骨遗迹: "Whalebone Ruins", //Way of Winter - 10/21/24
    瞭望点: "Lookout Point", //Way of Winter - 10/21/24
    路边野餐: "Roadside Picnic", //Way of Winter - 10/21/24
    古船农场: "Ancient Ship Farm", //Way of Winter - 10/21/24
    临时调查营地: "Temporary Investigation Camp", //Way of Winter - 10/21/24
    倒挂遗迹: "Inverted Ruins", //Way of Winter - 10/21/24
    幽灵渔村: "Ghost Bay", //Way of Winter - 10/21/24
    悬船冰洞: "Dangling Vessel Cove", //Way of Winter - 10/21/24
    断桥: "Collapsed Bridge", //Way of Winter - 10/21/24
    暖火营地: "Warmfire Camp", //Way of Winter - 10/21/24
    雪松小屋: "Cedar Cabin", //Way of Winter - 10/21/24

    // Securement Silos - 收容井
    "收容井-西格玛": "Securement Silo - Sigma",
    "收容井-斐": "Securement Silo - Phi",
    "收容井-暗狱": "Securement Silo EX1",
    "收容井-阿尔法": "Securement Silo - Alpha",
    "收容井-寒境": "Securement Silo - Psi",
    "收容井-西塔": "Securement Silo - Theta",
    "收容井·塔乌": "Securement Silo 08",

    // Rift Anchors - 位面锚 // 空间锚点 = Rift Anchor
    //Way of Winter - 雪国人生
    "空间锚点·断桥": "Rift Anchor - Collapsed Bridge", //Way of Winter - 10/21/24
    "空间锚点·“心愿奇境”": "Rift Anchor - Wishland - West", //Way of Winter - 10/21/24
    "空间锚点·慕斯市食品厂": "Rift Anchor - Mousseville Sweet Factory", //Way of Winter - 10/21/24
    "空间锚点·慕斯市": "Rift Anchor - Moousseville", //Way of Winter - 10/21/24
    "空间锚点·搁浅滩涂": "Rift Anchor - Stranded Marsh", //Way of Winter - 10/21/24
    "空间锚点·烟袋度假区": "Rift Anchor - Tabacco Resort", //Way of Winter - 10/21/24
    "空间锚点·悬船冰洞": "Rift Anchor - Dangling Vessel Cove", //Way of Winter - 10/21/24
    "空间锚点·菲斯库尔港": "Rift Anchor - Fiskur Harbor", //Way of Winter - 10/21/24
    "空间锚点·“母巢”收容保护区": "Rift Anchor - Hiverail Restricted Area", //Way of Winter - 10/21/24
    "空间锚点·黑石要塞": "Rift Anchor - Obsidian Camp", //Way of Winter - 10/21/24
    "空间锚点·“大河”收容场": "Rift Anchor - Taiga Containment Center", //Way of Winter - 10/21/24
    "空间锚点·冻火湾": "Rift Anchor - Frost Forge", //Way of Winter - 10/21/24
    "空间锚点·“黑域”之门": "Rift Anchor - Entrance of the Zone of Haze", //Way of Winter - 10/21/24
    "空间锚点·致命信号": "Rift Anchor - Fatal Signal", //Way of Winter - 10/21/24
    "空间锚点·蛇腹洞穴": "Rift Anchor - SunShroud Cave",
    "空间锚点·蛇尾滑索营地": "Rift Anchor - Serpent's Tail Camp",
    "空间锚点·燃尽之地": "Rift Anchor - Charred WasteLand",
    "空间锚点·废弃提炼厂": "Rift Anchor - Abandoned Refinery",
    "空间锚点·火喉下城": "Rift Anchor - Fire Throat Fortress",
    "空间锚点·灰落市1": "Rift Anchor - AshenTon 1",
    "空间锚点·灰落市2": "Rift Anchor - AshenTon 2",
    "空间锚点·炎神研究所": "Rift Anchor - Fire God Research Center",
    "空间锚点·温泉民宿": "Rift Anchor - Hot Spring Inn",
    "空间锚点·塔乌": "Rift Anchor - Securement Silo 08",

    // has no individual names yet in chinese
    // Mysterious Chests - 神秘宝藏
    // has no individual names
    // Weapon Crates - 武器箱
    // has no individual names
    "武器箱【剧情箱子】": "Weapon Crate【Story】",
    // Gear Crates - 装备箱
    // has no individual names
    // Hidden Crates - 隐藏宝箱
    // has no individual names
    // Morphic Crates - 宝箱怪
    // has no individual names
    // Storage Crates - 储物箱
    // has no individual names
    // Elites - 精英怪
    暴食狂: "Glutton",
    屠夫: "Arbiter",
    裁决者: "Arbiter",
    星尘簇: "Star Walker",
    门徒: "Disciple",
    雨壳: "Rainfall Reaper",
    腐坏眼虫: "Claw Machine", // Chagas Weevil
    娃娃机: "Claw Machine", // Chagas Weevil
    抓娃娃机: "Claw Machine", // Chagas Weevil
    唤生之母: "Mother of Life",
    唤生水母: "Mother of Life",
    熔烛: "Molton Candle",
    乐园向导: "Balloon Boy",
    夜魇: "Cervitaur",
    天气预报: "Weather Forecast",
    罪人守望: "Sinner Sentry",
    //"”教练“ 【清剿目标】": "", //removed?
    //'"龙脊" 科利·兰根': "", //removed?
    罗塞塔防暴员: "Rosetta Bruiser",
    罗塞塔机械师: "Rosetta Mechanic",
    罗塞塔重装员: "Rosetta Heavy",
    // Fog Events - 迷雾区域
    // has no individual names
    // Workshops - 防守工坊
    // has no individual names
    // Moving Houses - 移动房屋
    // has no individual names
    // Red Events - 限时事件
    这里没有迪斯科: "Where Does Disco Go?",
    湖之暗面: "The Dark Side",
    //"": "Cargo Scramble"
    挖通地狱: "Dig to Hell",
    腐乐园: "Paradise Lost",
    喀戎行动: "Operation Chiron",
    // Accessories - 武器配件
    // Weapon Attachments - 武器配件
    // Optics - 瞄具配件
    轻量化红点瞄具: "Light Reflex Sight",
    战斗侦查瞄具: "Recon Combat Optic",
    反射式瞄具: "Reflex Sight",
    精准步枪瞄具: "Precision Rifle Scope",
    战术全像瞄准镜: "Tactical Holographic Sight",
    精准光学瞄具: "Precision Optic",
    战术步枪瞄具: "Tactical Rifle Scope",
    轻型光学瞄具: "Light Optic",
    中距离战术瞄具: "Mid-Range Optic",
    密封型反射式瞄具: "Sealed Reflex Sight",
    紧凑型反射式瞄具: "Compact Reflex Sight",
    棱镜式瞄具: "Prismatic Sight",
    远距离战术瞄具: "Long-Range Tactical Sight",
    大型光学瞄具: "Large Optic Sight",
    紧固型微型红点瞄具: "Fastened Mini Reflex Sight",
    微型红点瞄具: "Mini Reflex Sight",
    先进战斗光学瞄具: "Advanced Combat Optic",
    全像瞄准镜: "Holographic Sight",
    // Muzzle Attachments - 枪口配件
    鱼鹰抑制器: "Osprey Suppressor",
    集成型补偿器: "Integrated Compensator",
    多用途霰弹枪抑制器: "Multi-Use Shotgun Suppressor",
    锤头型补偿器: "Hammer Compensator",
    短型抑制器: "Short Suppressor",
    战术消焰器: "Tactical Flash Hider",
    小型补偿器: "Small Compensator",
    加长型制退器: "Extended Brake",
    一体式抑制器: "Integrated Suppressor",
    小型制退器: "Small Muzzle Brake",
    一体式消焰器: "Integrated Flash Hider",
    //"多孔式消焰器": "", // Multi-Hole Flash Hider
    伪装用抑制器: "Camouflage Suppressor",
    大型抑制器: "Large Suppressor",
    大型枪口制退器: "Large Brake",
    前向消焰器: "Forward Flash Hider",
    消焰器: "Flash Hider",
    轻量化制退器: "Light Brake",
    基础抑制器: "Basic Suppressor",
    紧凑型制退器: "Compact Brake",
    霰弹枪制退器: "Shotgun Brake",
    // Tactial Attachments - 战术配件
    小型红外指示器: "Mini Reflex Sight, Small Infrared",
    前护手握把: "Front Guard Grip",
    后护手握把: "Back Guard Grip",
    大型激光指示器: "Large Laser Sight",
    集成型激光指示器: "Integrated Laser Sight",
    角型握把: "Triangle Grip",
    战术激光指示器: "Tactical Laser Sight",
    小型激光指示器: "Small Laser Sight",
    轻质前握把: "Light Front Grip",
    轻质水平握把: "Light Horizontal Grip",
    战术手电筒: "Tactical Flashlight",
    红外指示器: "Infrared Indicator",
    紧凑型红外指示器: "Compact Infrared Indicator",
    战术组合握把: "Tactical Combine Grip",
    眼镜蛇握把: "Cobra Grip",
    垂直握把: "Vertical Grip",
    紧凑型手电筒: "Compact Flashlight",
    激光指示器: "Laser Sight",
    折叠脚架: "Folding Bipod",
    // Magazines - 弹匣配件
    狙击枪战术弹匣: "Tactical SR Mag",
    轻机枪校准弹鼓: "Calibrated LMG Drum Mag",
    狙击枪改良弹匣: "Amended SR Mag",
    步枪轻量化弹匣: "Light Rifle Mag",
    冲锋枪加重型弹匣: "Weighted SMG Mag",
    轻机枪大型弹鼓: "Large LMG Drum Mag",
    手枪战术型弹匣: "Tactical Pistol Mag",
    冲锋枪战术型弹匣: "Tactical SMG Mag",
    步枪扩容弹匣: "Extended Rifle Mag",
    手枪加长型弹匣: "Extended Pistol Mag",
    手枪改良弹匣: "Amended Pistol Mag",
    冲锋枪改良弹匣: "Amended SMG Mag",
    步枪改良弹匣: "Amended Rifle Mag",
    // Puzzle Types
    异常门扉: "Door Puzzle",
    异常路牌: "Broken Billboard",
    广告牌异常: "Broken Billboard",
    破碎山石异常: "Breakable Rock",
    异常电话亭: "Phone Booth",
    异常雕像: "Statue Puzzle",
    黑水相机: "Camera Emote",
    //"": "Compact Brake (needs Mysterious Painting)",
    "诡谲画作: 小型制退器": "Small Muzzle Brake (needs Mysterious Painting)",
    "诡谲画作: 轻型光学瞄具": "Light Optic (needs Mysterious Painting)",
    "诡谲画作: 小型激光指示器": "Small Laser Sight (needs Mysterious Painting)",
    "诡谲画作: 战术步枪瞄具":
      "Tacticle Rifle Scope (needs Mysterious Painting)",
    "诡谲画作: 前向消焰器": "Forward Flash Hider (needs Mysterious Painting)",
    // Attachment Dupes with Spelling differences
    战斗侦察瞄具: "Recon Combat Optic",
    "反射式瞄具·黑色": "Reflex Sight",
    角形握把: "Triangle Grip",
    霰弹枪式制动器: "Shotgun Brake",
    密封型反射瞄具: "Sealed Reflex Sight",
    "多用途霰弹枪抑制器(6226,-588)": "Multi-Use Shotgun Suppressor (6226,-588)", // Keep coordinates when available
    "大型光学瞄具,黑心地带(3039,990)": "Large Optic Sight (3039,990)",
    "大型光学瞄具,黑心地带(3037,-979)": "Large Optic Sight (3037,-979)",
    手枪加强型弹匣: "Extended Pistol Mag",
    "诡谲画作：小型制退器": "Small Muzzle Brake (needs Mysterious Painting)",
    "诡谲画作：轻型光学瞄具": "Light Optic (needs Mysterious Painting)",
    "诡谲画作：小型激光指示器": "Small Laser Sight (needs Mysterious Painting)",
    "诡谲画作：战术步枪瞄具":
      "Tacticle Rifle Scope (needs Mysterious Painting)",
    "诡谲画作：前向消焰器": "Forward Flash Hider (needs Mysterious Painting)",
    "诡谲画作:战术步枪瞄具": "Tacticle Rifle Scope (needs Mysterious Painting)",
    // Item Categories - 手枪
    手枪: "Pistols",
    霰弹枪: "Shotguns",
    冲锋枪: "SMGs", // Submachine Guns
    步枪: "Rifles",
    狙击枪: "Sniper Rifles",
    轻机枪: "LMGs", // Light Machine Guns
    弓弩: "Crossbows",
    重型火器: "Heavy Weapons",
    刀剑: "Melee Weapons",
    头盔: "Helmets",
    面部: "Masks", // Face
    上身: "Vests", // Top
    下身: "Pants", // Bottom
    手套: "Gloves",
    鞋子: "Shoes",
    瞄具配件: "Optics",
    枪口配件: "Muzzle Attachments",
    战术配件: "Tactial Attachments",
    弹匣配件: "Magazines",
    碎弹模组: "Shrapnel Mods",
    霜寒模组: "Frost Vortex Mods",
    爆弹模组: "Unstable Bomber Mods",
    电涌模组: "Power Surge Mods",
    标记模组: "The Bull's Eye Mods",
    弹射模组: "Bounce Mods",
    快枪手模组: "Fast Gunner Mods",
    灼烧模组: "Burn Mods",
    阵地模组: "Fortress Warfare Mods",
    武器通用: "Weapon Compatible", // ?
    头盔模组: "Helmet Mods",
    面部模组: "Mask Mods", // Face mods
    上身模组: "Vest Mods", // Top mods
    下身模组: "Pants Mods", // Bottom Mods
    手套模组: "Glove Mods",
    鞋子模组: "Shoe Mods",
    防具通用: "Armor Compatible", // ?
    近战通用: "Melee General", // ?
    流派效果: "Genre Effect", // ?
    模组后缀: "Module Suffixes", // ?
    元素伤害: "Elemental Damage", // ?
    特殊子弹: "Special Bullets", // ?
    套装碎片: "Set Fragments",
    素材: "Materials",
    近战蓝图: "Melee Weapon Blueprints",
    手枪蓝图: "Pistol Blueprints",
    霰弹枪蓝图: "Shotgun Blueprints",
    冲锋枪蓝图: "SMG Blueprints", // Submachine Gun Blueprints
    步枪蓝图: "Rifle Blueprints",
    狙击枪蓝图: "Sniper Rifle Blueprints",
    轻机枪蓝图: "LMG Blueprints", // Light Machine Gun Blueprints
    弓弩蓝图: "Crossbow Blueprints",
    家具配方: "Furniture Blueprints",
    肉类: "Meat",
    鱼类: "Fish",
    药草: "Herbs",
    // Recipes - 食谱
    "食谱：熏肉罗宋汤": "Recipe: Borscht Deluxe",
    "食谱：畸变塔可卷": "Recipe: Taco", // In World item says Herbal Tea
    "食谱：星尘茶": "Recipe: Stardust Tea",
    "食谱：培根肉排汉堡": "Recipe: Bacon Burger",
    "食谱：南瓜星空派": "Recipe: Stargazy Pie",
    "食谱：消脂果汁": "Recipe: Fat-Burning Juice",
    "食谱：安全三明治": "Recipe: Safety Sandwich",
    "食谱：星尘水果罐头": "Recipe: Assorted Canned Food",
    "食谱：星尘通心粉罐头": "Recipe: Stardust Italian Soup Can",
    "食谱：奇迹茶": "Recipe: Miracle Tea",
    "食谱：碎裂面包": "Recipe: Shattered Bread",
    "食谱：星尘南瓜泥沙拉": "Recipe: Stardust Pumpkin Salad",
    "食谱：幽光蘑菇罐头": "Recipe: Spectral Canned Mushroom",
    "食谱：星尘炸肉": "Recipe: Mixed Fried Hot Dog",
    "食谱：元宝饺子": "Recipe: Yuanbao Dumplings",
    "食谱：深海鱼片披萨": "Recipe: Stargazy pizza",
    "食谱：炸薯条": "Recipe: French Fries",
    "食谱：反重力奶昔": "Recipe: Anti-gravity Milkshake",
    "食谱：金钱饼": "Recipe: Croquette",
    "食谱：特调冰酿": "Recipe: Signature Ice Brew",
    "食谱:奇思妙想饮": "Recipe: Whimsical Drink",
    "食谱：炸鱼薯条": "Recipe: Fish & Chips",
    "食谱：幽灵饼干": "Recipe: Ghost Cookies",
    "食谱：畸变带骨香肠": "Recipe: Bone-in Deviated Sausage",
    "食谱：捕获之汤": "Recipe: Securement Soup",
    "食谱：蜜酿炖肉": "Recipe: Honey Glaced Meat",
    // Deviants - 收容物
    远归之蝶: "Butterfly's Emissary",
    活体凝胶: "Festering Gel",
    荣枯种子: "Harveseed",
    捕梦网: "Dreamcatcher",
    低语亡骨: "Canine Skull",
    电鳗: "Electric Eel",
    迪斯科球: "Disco Ball",
    发条青蛙: "Frog the Leaper",
    极寒水母: "Polar Jelly",
    青龙: "Chefosaurus",
    蜂团团: "Buzzy Bee",
    工号37: "H37",
    //"": "Fetch-A-Lot Bunny",
    姜饼屋: "Gingerbread House",
    核子打火机: "Atomic Lighter",
    凝视的黑猫: "Enchanted Void",
    木工河狸: "Logging Beaver",
    空间魔方: "Space Turner",
    元素小子: "Digby Boy",
    雪景球: "Snow Globe",
    上界之卵: "Upper World Spawn",
    庞然大菇: "Growshroom",
    皆斩: "Zeno-Purifier",
    门扉: "Strange Door",
    //"": "Party Monkey",
    巫毒娃娃: "Voodoo Doll",
    炎之精: "Flame Essence",
    破碎少女: "Shattered Maiden",
    迷你奇点: "Mini Wonder",
    异维大猫: "Extradimensional Cat",
    球状闪电: "Orb Lightning",
    迷你黄衣: "Mini Feaster",
    小纸人: "Paper Doll",
    //"": "Lethal Rabit",
    //"": "Artisan's Touch",
    愿望先生: "Mr Wish",
    红龙: "Pyro Dino",
    熊医生: "Dr. Teddy",
    恶臭球根: "Grumpy Bulb",
    空之子: "By-The-Wind",
    //"": "Lava Fox",
    胡桃夹子: "Nutcracker",
    //"": "Wish Machine",
    蝶之梦: "Rebecca",
    气球狗: "Pup Buddy",
    一碗美味: "Hug-In-A-Bowl",
    //"": "Masonic Pyramid",
    // Mysterious Painting - 诡谲画作
    "诡谲画作·代顿湿地": "Mysterious Painting · Dayton Wetlands",
    "诡谲画作·破碎三角洲": "Mysterious Painting · Broken Delta",
    "诡谲画作·铁锈河": "Mysterious Painting · Iron River",
    "诡谲画作·垩峰山": "Mysterious Painting · Chalk Peak",
    "诡谲画作·孤狼荒原": "Mysterious Painting · Red Sands",
    "诡谲画作·黑心地带": "Mysterious Painting · Blackheart",
    // Ores
    //"": "Rock",
    硫磺: "Sulfur",
    铜矿: "Copper Ore",
    锡矿: "Tin Ore",
    锡矿富集区: "Tin Ore Area",
    铁矿: "Iron Ore",
    铝矿: "Aluminum Ore",
    钨矿: "Tungsten Ore",
    银矿: "Silver Ore",
    银矿区域: "Silver Ore Area",
    "银矿(稀有)": "Silver Ore (Random)",
    "银矿 (稀有)": "Silver Ore (Random)",
    金矿: "Gold Ore",
    金矿区域: "Gold Ore Area",
    "金矿(稀有)": "Gold Ore (Random)",
    "金矿 (稀有)": "Gold Ore (Random)",
    "金矿（先锋服)": "Gold Mine",
    星尘矿: "Stardust Ore",
    星尘矿石: "Stardust Ore",
    星尘矿区: "Stardust Ore Area",
    星尘矿区域: "Stardust Ore Area",
    "星尘矿(稀有)": "Stardust Ore (Random)",
    "星尘矿 (稀有)": "Stardust Ore (Random)",
    // Plants
    橘子: "Oranges",
    果蔬篮橘子: "Oranges",
    小麦: "Wheat",
    果蔬篮小麦: "Wheat",
    红色浆果: "Hawthorn Berry",
    卷心菜: "Cabbage",
    果蔬篮卷心菜: "Cabbage",
    甜菜: "Beet",
    果蔬篮甜菜: "Beet",
    蓝莓: "Blueberry",
    南瓜: "Pumpkin",
    果蔬篮南瓜: "Pumpkin",
    玉米: "Corn",
    果蔬篮玉米: "Corn",
    土豆: "Potato",
    果蔬篮土豆: "Potato",
    蘑菇: "Mushroom",
    果蔬篮蘑菇: "Mushroom",
    火刺果: "Spikemato", //Way of Winter - 10/22/24
    阳姜: "Sunny Ginger", //Way of Winter - 10/22/24
    薄荷: "Mint", //Way of Winter - 10/22/24
    雪茸: "Snow Fungus", //Way of Winter - 10/22/24
    //"": "Cucumber",
    //"": "Tomato",
    //"": "Red Tulip",
    //"": "Yellow Tulip",
    //"": "Yellow Hydrangea",
    //"": "Blue Hydrangea",
    //"": "White Petunia",
    //"": "Blue Petunia",
    芦荟: "Aloe Vera",
    "鼠尾草 * 5": "Sage * 5",
    "鼠尾草 * 8": "Sage * 8",
    桔梗: "Bellflower",
    金银花: "Honeysuckle",
    "金银花 * 3": "Honeysuckle * 3",
    "金银花 * 4": "Honeysuckle * 4",
    藏红花: "Saffron",
    血菖蒲: "Scarlet Calamus",
    紫锥菊: "Coneflower",
    黄莲花: "Yellow Lotus",
    //"": "Peppercorn",
    苹果: "Apple",
    苹果树: "Apple Tree",
    "苹果树 * 2": "Apple Tree * 2",
    //"": "Cactus",
    葡萄: "Grapes",
    贝壳岩石: "Shell Rock",
    海草岩石: "Seaweed Rock",
    观赏性花海: "Sea of ​​Flowers",
    香蕉树: "Banana Tree",
    "香蕉树 * 2": "Banana Tree * 2",
    "香蕉树 * 4": "Banana Tree * 4",
    "香蕉树 * 6": "Banana Tree * 6",
    "香蕉树 * 7": "Banana Tree * 7",
    椰子树: "Coconut Tree",
    "椰子树 * 3": "Coconut Tree * 3",
    巨木: "Giant Tree",
    "龙血树(巨木)": "Giant Tree",
    // Animals
    野生动物: "Animals",
    兔子窝: "Rabbit Hole",
    鹿: "Deer",
    星蚀鹿群: "Deer",
    星蚀雄鹿: "Deer",
    星蚀雌鹿: "Deer",
    野猪: "Boar",
    野猪群: "Boar",
    星蚀鳄: "Alligator",
    羊群: "Flock",
    花豹: "Leopard",
    星蚀熊窝: "Bear's Den",
    星蚀熊: "Bear",
    肉排: "Steak",
    鳄鱼蛋: "Alligator Eggs",
    鸟蛋: "Bird Eggs",
    // Oil Wells
    原油油井: "Oil Well",
    // Echo Stone - 回响石
    // has no individual names
    // Hints - 留言
    别靠近箱子: "Mimic",
    地堡门禁卡: "Bunker Access Card",
    吓人箱子: "Mimic",
    空间异动: "Space Anomaly",
    地下入口: "Underground Entrance",
    电话亭: "Telephone Booth",
    拟态怪储物箱: "Mimic",
    拟态宝箱怪: "Mimic",
    "密码: 5897": "Password: 5897",
    "密码: 325": "Password: 325",
    地下通道入口: "Underground Entrance",
    地下通道最近入口: "Underground Entrance",
    地窖入口: "Underground Entrance",
    山洞入口: "Cave Entrance",
    溶洞入口: "Cave entrance",
    镜女: "Mirror Enemy",
    拟态储物箱别开: "Mimic",
    "电梯：负一层": "Elevator: Underground Level 1",
    拟态宝箱怪别开: "Mimic",
    空间异常: "Space Anomaly",
    "5层楼梯": "5 Stairs",
    箱子怪别开: "Mimic",
    下楼电梯: "Elevator",
    下楼电梯位置: "Elevator",
    隐藏宝箱最后位置: "Treasure Chest",
    抽屉柜: "Drawers",
    混合矿场: "Mixed Mining",
    // Side Quests - 隐藏任务
    //"": "A Miracle Chase",
    隐藏的宝箱: "Morphic Crate Tutorial", //
    //"异常档案": "",//Abnormal Files?
    //"": "The Shadowed Lighthouse",
    玩具小熊: "The Call of Childhood",
    "急救！急救！": "Dayton Hospital",
    //'"玩具熊"女孩': "", //Teddy Bear Girl?
    //"": "Harvest Season",
    //"": "The Lost Lady",
    //"": "Deviation Expert",
    //"": "Martial God's Home",
    信徒的藏宝图: "Lost In Madness - Starting Point",
    信徒的财产箱: "Lost In Madness - Treasure Location",
    //"": "Stardust Infiltration",
    //"": "An Act of Theft",
    检测点见闻: "Sketch of an Inspection Point",
    //"": "Supply Line",
    //"": "Where the Cat and Mice Play",
    红与黑: "Carnival of Doom",
    //"": "The Treasure Infinite",
    //"": "Love Sorrow",
    //"": "Foreign Rock",
    //"": "Unfinished Expedition",
    //"": "Mayfly Badge",
    //"": "Secret Flavors",
    //"": "Save Meta-Human",
    //"": "Welcome!",
    //"": "Boy Just Wanna Have Fun",
    //"": "For Sarah",
    //"": "A Gift Overdue",
    //"": "Experimental Material",
    //"女儿的问候": "", //Greetings from my daughter?
    //"": "Into the Madness",
    //"": "Grumbling Scavenger",
    //"": "To Save or Not to Save",
    //"寻找记忆": "", //Searching for memories?
    //"": "Save Meta-Human",
    被丢弃的玩偶: "Anna's Toys",
    //"": "Chance Encounter: Welcome Aboard",
    艺术家的钥匙: "Cultist Meeting - Starting Point",
    信徒的宝箱: "Cultist Meeting - Treasure Location",
    特别关系: "A Special Relationship",
    //"嘉莲": "",
    生日快乐: "Happy Birthday",
    //"": "Little Shop of Kindness",
    招财猫开始点: "Lucky Cat - Starting Point",
    招财猫1号: "Lucky Cat #1",
    招财猫2号: "Lucky Cat #2",
    招财猫3号: "Lucky Cat #3",
    招财猫给予的宝箱: "Lucky Cat - Treasure Location",
    好消息坏消息: "On Joy and Sorrow - Starting Point",
    神婆的宝箱: "On Joy and Sorrow - Treasure Location",
    //"成就：一览众山小": "",//Achievement: A view of the world?
    农庄惊魂: "Farm Horror",
    限定版牛奶: "Limited Edition",
    考古学家的馈赠: "Be True 'Til Death",
    遗留财产: "A Family Legacy",
    悲伤画像: "Sorrowful Portrait",
    //"罗塞塔保险箱": "", //Rosetta Safe?
    "密码：9274": "Password: 9274", //Really a Side Quest?
    秘密配方: "Recipe", //Really a Side Quest?
    // Merchant - 商人
    韦伯: "Weber",
    赵金潭: "Zhao Jintan",
    //"": "Jonathan",
    //"": "Emma Potter",
    拟态体商店: "Severin VII-S2041 (SIRA Trader)",
    //"": "Nisa (Sproutlet Trader)",
    鱼饵商人: "Margerie (Fish Merchant)",
    "鱼饵商人【霜之白班钩鱼蓝图兑换处】": "Margerie【Fish Blueprints】",
    // Livable Location - 宜居地点
    // Livable Location are the recommended places to live with subjective opinions, These are usually large flat areas with rich resources and close to water sources.
    // has no individual names
    // Not sure where/if the following are used.
    异常点: "Abnormal Object",
    稀有: "Rare",
    持续刷新: "Continuous Refresh",
    碧湖山庄: "Greenlake Hill",
    检测点91: "Inspection Point 91",
    戴维思角: "Davis Point",
    检测点24: "Inspection Point 24",
    翻车鱼岛旅客中心: "Mola Island Visitor Center",
    治炼厂污染点: "Refinery Pollution Plan",
    检测点3711: "Inspection Point 3711",
    // 2024/08/17
    翠湖山镇: "Greenlake Hill",
    无蘑菇: "No Mushroom",
    果蔬箱土豆: "Potato",
    油田区域: "Oilfield Area",
    抽水泵采集: "Pump",
    洞穴内: "Inside Cave",
    便携式燃油: "Portable Fuel",
    腐化熊: "Bear",
    水豚: "Capybara",
    狼: "Wolf",
    鳄鱼: "Crocodile",
    长嘴海鸥: "Long-Billed Seagull",
    海龟: "Sea Turtle",
    鹈鹕: "Pelican",
    兔子: "Rabbit",
    鳄龟: "Snapping Turtle",
    狐狸: "Fox",
    浣熊: "Raccoon",
    火烈鸟: "Flamingo",
    腐化鳄: "Corrupted Crocodile",
    腐化狼: "Corrupted Wolf",
    白斑狗鱼钓鱼点: "Northern Pike",
    白斑狗鱼: "Northern Pike",
    海鲈鱼: "Sea Bass",
    神仙鱼: "Angelfish",
    长吻雀鳝: "Longnose Gar",
    黑鲈: "Black Bass",
    孔雀鱼: "Guppy",
    鲤鱼: "Carp",
    图丽鱼: "Astronotus Ocellatus",
    太阳鱼: "Lepomis",
    锦鲤: "Koi",
    龙鱼: "Arowana",
    鳙鱼: "Bighead Carp",
    沙丁鱼: "Sardine",
    梭鱼: "Barracuda",
    鲭鱼: "Mackerel",
    罗非鱼: "Tilapia",
    虹鳟: "Rainbow Trout",
    鲑鱼: "Salmon",
    鲳鲹: "Pompano",
    松鼠: "Squirrel",
    亚口鱼: "Sucker",
    // Scenario
    "PvE-天空之触": "PvE-MANIBUS",
    "PvP-分歧之路": "PvP-Evolution's Call",

    "PvP-逆位战争": "PvP-Prismverse's Clash", // 10/19/24
    "PvE-雪国人生": "PvE-Way of Winter", // 10/19/24
    // map
    玩家: "Player",

    私有收容仓: "Private Securement Unit",
    出生点: "Spawn Point",
    //维纳峡湾
    "观景点：雪崩巨楔": "ViewPoint: Avalanche Monolith",
    "观景点：菲斯库尔港": "ViewPoint: Fiskur Harbor",
    "观景点：熊栖雪场": "ViewPoint: Bear's Den",
    "观景点：母巢": "ViewPoint: Hiverail",
    "观景点：白崖黑域": "ViewPoint: White Cliff Black Sector",
    "观景点：心愿奇境西园": "ViewPoint: Wish Land - West",
    "观景点：荒原巨骨": "ViewPoint: Wasteland Skull",
    "观景点：冻火湾": "ViewPoint: Forst Forge",
    "观景点：巨岩": "ViewPoint: Boulder",
    "观景点：鲸骨乡": "ViewPoint: Whalebone",
    //玛瑙冻原
    "观景点：“大河”收容场": "ViewPoint: TAIGA Containment Center",
    "观景点：搁浅船只": "ViewPoint: Stranded Ship",
    "观景点：慕斯市食品厂": "ViewPoint: Mousseville Sweet Factory",
    "观景点：冻原巨楔": "ViewPoint: Tundra Monolith",
    "观景点：慕斯市": "ViewPoint: Mousseville",
    "观景点：心愿奇境东园": "ViewPoint: Wish Land Eastern Park",
    "观景点：黑石要塞": "ViewPoint: Fort Obsidian",
    "观景点：绚烂花海": "ViewPoint: Flower Fields",
    "观景点：冰屋营地": "ViewPoint: Camp Igloo",
    //余烬滩
    "观景点：炎神实验室": "ViewPoint: Fire God Research Center",
    "观景点：不落号坠落点": "ViewPoint: Ground Unfallen",
    "观景点：收容井塔乌": "ViewPoint: Securement Silo Taurus",
    "观景点：火喉下城": "ViewPoint: Fire Throat Fortress",
    "观景点：炽尘巨楔": "ViewPoint: Emberdust Monolith",
    "观景点：火喉圣域": "ViewPoint: Fire Throat Sanctum",
    "观景点：月牙湖": "ViewPoint: Crescent Lake",
    "观景点：灰落": "ViewPoint: Ashenton",
    "观景点：影日洞穴": "ViewPoint: Sunshroud Cave",
    "观景点：遗忘之地": "ViewPoint: Oblivion",

    //Way of Winter - 10        "荒废农庄": "",/21/24

    "空间锚点·废弃矿洞二层": "Rift Anchor - Abandoned Mine Floor 2",
    "收容井-德尔塔": "Securement Silo - Delta",
    电梯: "elevator",
    乖乖兔: "Fetch-A-Lot Bunny",
    "食谱：黄油松茸": "Recipe: Butter Matsutake Mushrooms",
    "食谱：硬邦邦姜糖": "Recipe: GingerDrop",
    "食谱：蜜蛋弹弹乐": "Recipe: Ginger Poppers",
    "食谱：雪茸蛋炒饭": "Recipe: Snow Mushroom Fried Rice",
    "食谱：水果派": "Recipe: Mixed Fruit Pie",
    "食谱：噼里啪啦南瓜汤": "Recipe: Sparkling Pumpkin Soup",
    "食谱：炙热炖肉火锅": "Recipe: Hearty Meat Stew",
    "食谱：鲜肉咖喱饭": "Recipe: Curry and Rice",
    "食谱：砰砰爆米花": "Recipe: Popcorn",
    "食谱：水果蛋糕": "Recipe: Fruit Cake",
    "食谱：炙热烈焰蛋挞": "Recipe: Flaming Eggtart",
    "食谱：麻辣兔丁罐头": "Recipe: Canned Spicy Rabbit Dices",
    "食谱：清凉薄荷肉罐头": "Recipe: Canned Minty Meat",

    宝箱怪位置待定: "宝箱怪位置待定",
    "气球异常：轻量化红点瞄具": "气球异常：轻量化红点瞄具",
    "玩偶异常：戍卫前握把": "玩偶异常：戍卫前握把",
    "凝石异常：斜切握把": "凝石异常：斜切握把",
    "凝石异常：远射补偿器": "凝石异常：远射补偿器",
    "凝石异常：小型补偿器": "凝石异常：小型补偿器",
    "冰块宝箱：轻型光学瞄具": "冰块宝箱：轻型光学瞄具",
    "冰块宝箱：手枪加长型弹匣": "冰块宝箱：手枪加长型弹匣",
    "冰块宝箱：步枪扩容弹匣": "冰块宝箱：步枪扩容弹匣",
    "凝石异常：轻型握把": "凝石异常：轻型握把",
    "冰块宝箱：冲锋枪加重型弹匣": "冰块宝箱：冲锋枪加重型弹匣",
    "凝石异常：隐秘抑制器": "凝石异常：隐秘抑制器",
    "冰块宝箱：轻机枪大型弹鼓": "冰块宝箱：轻机枪大型弹鼓",
    "凝石异常：一体式消焰器": "凝石异常：一体式消焰器",
    "冰块宝箱：手枪战术型弹匣": "冰块宝箱：手枪战术型弹匣",
    "凝石异常：冲锋枪战术型弹匣": "凝石异常：冲锋枪战术型弹匣",
    "凝石异常：奇袭制退器": "凝石异常：奇袭制退器",
    "凝石异常：步枪轻量化弹匣": "凝石异常：步枪轻量化弹匣",
    "凝石异常：密封型反射式瞄具": "凝石异常：密封型反射式瞄具",
    "气球异常：消焰器": "气球异常：消焰器",
    "气球异常：小型制退器": "气球异常：小型制退器",
    "冰块宝箱：狙击枪战术弹匣": "冰块宝箱：狙击枪战术弹匣",
    "脚印异常：精准步枪瞄具": "脚印异常：精准步枪瞄具",
    "气球异常：狙击枪改良弹匣": "气球异常：狙击枪改良弹匣",
    "气球异常：小型红外指示器": "气球异常：小型红外指示器",
    "气球异常：垂直握把": "气球异常：垂直握把",
    "松鼠异常：棕熊反射式瞄具": "松鼠异常：棕熊反射式瞄具",
    观景台: "Scenic Viewpoint",
    气球异常: "balloon anomaly",
    玩偶异常: "doll anomaly",
    冰块宝箱: "ice Chest",
    冷晶矿: "Cold Crystal Ore",
    冰晶矿: "Cold Crystal Ore",
    热岩矿: "Hot Rock Ore",
    寒冰瓜: "Ice Melon",
    乌鸦蛋: "Raven Egg",
    极地椒: "Polar Pepper",
    "极地椒 * 3": "Polar Pepper * 3",
    驯鹿: "Reindeer",
    野牛: "Bison",
    海豹: "Seal",
    母羊: "Ewe",
    公羊: "Ram",
    北极狐: "Fox",
    北极熊: "Polar Bear",
    熊: "Bear",

    // 2025-04-04
    预兆宝箱: "Lunar Oracle Crates",
    代顿检测点传送塔: "Dayton Inspection Point Teleportation Tower",
    内海传送塔: "Inner Sea Teleportation Tower",
    "金银花*8": "Honeysuckle*8",
    上位者的影响使星尘污染浓度上升: "Polluted Area",
    这里的星尘污染浓度似乎很高: "Polluted Area",
    黑湖中不断溢出星尘污染: "Polluted Area",
    矿井内似乎不断溢出星尘污染: "Polluted Area",
    溢出的污染不断侵蚀着这片土地: "Polluted Area",
    狂信者的祝祷召唤出肆虐的污染: "Polluted Area",
    源质挥发使星尘污染浓度上升: "Polluted Area",
    山壁内源源不断渗出星尘污染: "Polluted Area",
    鳄龟蛋: "Snapping Turtle Eggs",
    "PvE-无尽长梦": "PvE-Endless Dream",
    无尽长梦: "Endless Dream",
    "PvP-禁区": "PvP-RaidZone",
    "七日世界：禁区": "OnceHuman: RaidZone",
    异常物纪元: "Era of Deviations",
    "PvE-异常物纪元": "PvE-Era of Deviations",
    贸易区: "Player Market",
    月之幽影: "月之幽影",
    月兆精英怪: "月兆精英怪",
    豹子: "Leopard",
    龙血树: "Dragon Blood Tree",
    古老者的试炼: "The Ancient One Trial",
    超异常体: "Super Anomaly",
    星临宝箱: "Starfall Crate",
    不稳定聚合体: "Unstable Aggregate ",
    引力异化精英怪: "Starfall Elite",
    剧情笔记: "Plot Notes",
    金色垃圾: "Golden Trash",
    浅眠记录: "Dozer",
    深眠记录: "Sleeper",
    永眠记录: "Dormant",
    噩梦记录: "Nightmare Tube",
    梦域商人: "Miss Memory",
    梦域追迹箱: "Dream Zone Trace Box",
    清醒时钟: "Wakeful Clock",
    野生装备台: "Gear Workbench",
    野生工艺台: "Supplies Workbench",
    野生车辆: "Vehicle Ruins",
    野生商人: "Merchant",
    矿石: "Minerals",
    植物: "Plants",
    动物: "Animals",
    蔬菜: "Vegetables",
    草药: "Herbs",
    油田: "Oil Field",

    // "Deviation: Survive"
    非正常收容: "Deviation: Survive",
    竞技场: "Arena",
    野生收容物: "Wild Deviation",
    斗技馆: "Gym",
    许愿者雕像: "Strange Statue",
    新绿竞技场: "Verdant Arena",
    热泉竞技场: "Hot Springs Arena",
    冬灵竞技场: "Snowsprite Arena",
    心愿竞技场: "Wish Arena",
    "远归之蝶 (15级)": "Butterfly's Emissary (lv.15)",
    "远归之蝶 (22级)": "Butterfly's Emissary (lv.22)",
    "远归之蝶 (28级)": "Butterfly's Emissary (lv.28)",
    "远归之蝶 (37级)": "Butterfly's Emissary (lv.37)",
    "元素小子 (B级)": "The Digby Boy (В)",
    "元素小子 (5级)": "The Digby Boy (lv.5)",
    "元素小子 (15级)": "The Digby Boy (lv.15)",
    "元素小子 (22级)": "The Digby Boy (lv.22)",
    "元素小子 (25级)": "The Digby Boy (lv.25)",
    "元素小子 (28级)": "The Digby Boy (lv.28)",
    "元素小子 (43级)": "The Digby Boy (lv.43)",
    "元素小子 (43级)": "The Digby Boy (lv.43)",
    "元素小子 (45级)": "The Digby Boy (lv.45)",
    "夺命兔 (B)": "Lethal Rabbit (B)",
    "夺命兔 (15级)": "Lethal Rabbit (lv.15)",
    "夺命兔 (40级)": "Lethal Rabbit (lv.40)",
    "夺命兔 (45级)": "Lethal Rabbit (lv.45)",
    "乖乖兔 (B)": "Fetch-A-Lot Bunny (B)",
    "永恒烈阳 (A)": "Invincible Sun (А)",
    "派对猴 (B)": "Party Monkey (B)",
    "庞然大菇 (5级)": "Growshroom (lv.5)",
    "庞然大菇 (22级)": "Growshroom (lv.22)",
    "庞然大菇(22级)": "Growshroom (lv.22)",
    "庞然大菇 (40级)": "Growshroom (lv.40)",
    "庞然大菇 (43级)": "Growshroom (lv.43)",
    "庞然大菇 (45级)": "Growshroom (lv.45)",
    "量子蜗牛 (5级)": "Atomic Snail (lv.5)",
    "量子蜗牛 (15级)": "Atomic Snail (lv.15)",
    "量子蜗牛 (22级)": "Atomic Snail (lv.22)",
    "量子蜗牛 (28级)": "Atomic Snail (lv.28)",
    "量子蜗牛 (35级)": "Atomic Snail (lv.35)",
    "低语孤狼 (5级)": "Lonewolf's Whisper (lv.5)",
    "低语孤狼 (15级)": "Lonewolf's Whisper (lv.15)",
    "低语孤狼 (22级)": "Lonewolf's Whisper (lv.22)",
    "低语孤狼 (28级)": "Lonewolf's Whisper (lv.28)",
    "蜂团团 (5级)": "Buzzy Bee (lv.5)",
    "蜂团团 (22级)": "Buzzy Bee (lv.22)",
    "蜂团团 (43级)": "Buzzy Bee (lv.43)",
    "蜂团团 (45级)": "Buzzy Bee (lv.45)",
    "永恒烈阳 (27级)": "Invincible Sun (lv.27)",
    "迪斯科球  (27级)": "Disco Ball (lv.27)",
    "迪斯科球(27级)": "Disco Ball (lv.27)",
    "迪斯科球 (B)": "Disco Ball (B)",
    "电鳗  (27级)": "Electric Eel (lv.27)",
    "电鳗 (B)": "Electric Eel (B)",
    "活性凝胶 (B)": "Festering Gel (B)",
    "活性凝胶 (25级)": "Festering Gel (lv.25)",
    "小纸人 (B)": "Paper Doll (B)",
    "核子打火机 (A)": "Atomic Lighter (A)",
    "捕梦网 (B)": "Dreamcatcher (B)",
    "熊医生 (A)": "Dr. Teddy (A)",
    "工号37 (A)": "Н37 (A)",
    "雨人 (15级)": "Rain Man (lv.15)",
    "雨人 (28级)": "Rain Man (lv.28)",
    "雨人 (37级)": "Rain Man (lv.37)",
    "雨人 (40级)": "Rain Man (lv.40)",
    "雨人 (45级)": "Rain Man (lv.45)",
    "气球狗 (A)": "Pup Buddy (A)",
    "发条青蛙 (5级)": "Frog the Leaper (lv.5)",
    "发条青蛙 (22级)": "Frog the Leaper (lv.22)",
    "发条青蛙 (28级)": "Frog the Leaper (lv.28)",
    "发条青蛙 (43级)": "Frog the Leaper (lv.43)",
    "恶咒娃娃 (A)": "Voodoo Doll (A)",
    "破碎少女 (B)": "Shattered Maiden (B)",
    "空间魔方 (A)": "Space Turner (A)",
    "联合体 (B)": "Masonic Pyramid (B)",
    "木工河狸 (B)": "Logging Beaver (В)",
    "木工河狸 (15级)": "Logging Beaver (lv.15)",
    "木工河狸 (22级)": "Logging Beaver (lv.22)",
    "木工河狸 (28级)": "Logging Beaver (lv.28)",
    "木工河狸 (45级)": "Logging Beaver (lv.45)",
    "庞然大菇 (28级)": "Growshroom (lv.28)",
    "炎之精 (22级)": "Flame Essence (lv.22)",
    "炎之精 (25级)": "Flame Essence (lv.25)",
    "炎之精 (28级)": "Flame Essence (lv.28)",
    "空之子 (22级)": "By-the-Wind (lv.22)",
    "空之子 (43级)": "By-the-Wind (lv.43)",
    "极寒水母 (22级)": "Polar Jelly (lv.22)",
    "极寒水母 (37级)": "Polar Jelly (lv.37)",
    "极寒水母 (40级)": "Polar Jelly (lv.40)",
    "极寒水母 (43级)": "Polar Jelly (lv.43)",
    "极寒水母 (45级)": "Polar Jelly (lv.45)",
    "胡桃夹子 (A)": "Nutcracker (А)",
    "雪景球 (B)": "Snow Globe (B)",
    "雪景球 (35级)": "Snow Globe (lv.35)",
    "姜饼屋 (A)": "Gingerbread House (А)",
    "快照 (A)": "ZapCam (А)",
    "冰之瓶 (B)": "Ice Pot (В)",
    "门扉 (S)": "Strange Door (S)",
    "鱼航员 (43级)": "Hydronaut Fish (lv.43)",
    "鱼航员 (45级)": "Hydronaut Fish (lv.45)",
    "鲸狗 (5级)": "Whalepup (lv.5)",
    "鲸狗 (22级)": "Whalepup (lv.22)",
    "鲸狗 (28级)": "Whalepup (lv.28)",
    "狐主任 (15级)": "Director Fox (lv.15)",
    "狐主任 (22级)": "Director Fox (lv.22)",
    "狐主任 (28级)": "Director Fox (lv.28)",
    "狐主任 (37级)": "Director Fox (lv.37)",
    "狐主任 (45级)": "Director Fox (lv.45)",
    "焦油布丁 (22级)": "Tar Pudding (lv.22)",
    "唤生灵 (37级)": "Soul Summoner (lv.37)",
    三角原传送塔: "Teleportation Tower",
    油田北部传送塔: "Teleportation Tower",
    秽水工业区: "Greywater Industrial Zone",
    铁锈河下游: "Iron River Lower Reaches",
    铁锈河上游: "Iron River Upper Reaches",
    黑原地区: "Blackfell Area",
    黑原北部: "Blackfell North",
    黑原西部: "Blackfell West",
    黑石山脉: "Blackstone Mountains",
    阿克姆山脉: "Arkam Mountains",
    冰屋湾: "Igloo Bay",
    乐园腹地: "Paradise Hinterland",
    不冻港地区: "Unfrozen Port Area",

    // 乐园岛
    乐园岛: "Paradise Island",
    金宝箱: "Gold Loot Crate",
    银宝箱: "Silver Loot Crates",
    "金属抽屉柜": "Metal Drawer Cabinet",
    "车后备箱": "Car Trunk",
    "垃圾箱": "Trash Can",
    "蛮荒地带": "Wild Deviant",
    "幻境地带": "Mirror Deviant",
    "虚实地带": "Phantasmal Deviant",
    "文明遗产": "Cultural Artifacts"
  },
};
